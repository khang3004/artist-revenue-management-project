-- =============================================================================
-- V2__ISA_Artists.sql (REFACTORED)
-- ISA using subtype tables ONLY (no redundant role table)
-- =============================================================================
...
(
    ...
    -- ❌ removed member_count (derived data)
...
(
    ...
    num_compositions INTEGER NOT NULL DEFAULT 0
        CHECK (num_compositions >= 0)
);
...
(
    band_id   INTEGER NOT NULL
        REFERENCES bands (artist_id) ON DELETE CASCADE,
    ...
    -- prevent band being its own member
    CHECK (band_id <> artist_id),
...
COMMENT ON COLUMN band_members.internal_split_pct IS 'Member share of band revenue (0–1).';

-- =============================================================================
-- INDEXES
-- =============================================================================
CREATE INDEX idx_band_members_band 
    ON band_members (band_id);

CREATE INDEX idx_band_members_artist 
    ON band_members (artist_id);

CREATE INDEX idx_band_members_band_artist 
    ON band_members (band_id, artist_id);